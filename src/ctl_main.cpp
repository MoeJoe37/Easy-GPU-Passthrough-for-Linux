...
static int printTextForCommand(const QString &cmd, const QJsonObject &obj) {
    ...
    if (cmd == "diagnose" || cmd == "preflight" || cmd == "laptopCheck" || cmd == "laptop-check") {
        if (cmd == "laptopCheck" || cmd == "laptop-check") {
            qInfo().noquote() << "Laptop compatibility:" << obj.value("summary").toString();
            qInfo().noquote() << "Likely laptop:" << (obj.value("likelyLaptop").toBool() ? "yes" : "no");
            qInfo().noquote() << "Target drives display:" << (obj.value("targetDrivesDisplay").toBool() ? "yes" : "no");
            qInfo().noquote() << "Target drives internal panel:" << (obj.value("targetDrivesInternalPanel").toBool() ? "yes" : "no");
            qInfo().noquote() << "Hybrid/offload candidate:" << (obj.value("hybridOffloadCandidate").toBool() ? "yes" : "no");
            qInfo().noquote() << "";
        }
...
int main(int argc, char **argv) {
    ...
    if (args.size() < 2) {
        qCritical() << "Usage: gpu-switcher-ctl <status|probe|inventory|diagnose|laptopCheck|autoSetupSingleGpu|simulate|simulateVm|simulateHost|installHook|switchToVm|switchToHost|rebootToVm|rebootToHost|restartHostNow|returnHostNextRestart|keepGpuForVm|safetyRecoverHostNow|applyBootState|on-vm-stopped> [gpuBdf] [--json]";
        return 1;
    }
    ...
    if (cmd == "autoSetupSingleGpu") {
        ...
        if (!gpuBdf.isEmpty()) req["gpuBdf"] = gpuBdf;
    } else if (cmd == "laptopCheck" || cmd == "laptop-check") {
        req = QJsonObject{{"cmd", "laptopCheck"}};
        if (!gpuBdf.isEmpty()) req["gpuBdf"] = gpuBdf;
    ...
    if (jsonOut || (cmd != "inventory" && cmd != "diagnose" && cmd != "preflight" && cmd != "laptopCheck" && cmd != "laptop-check" && !cmd.startsWith("simulate"))) {
